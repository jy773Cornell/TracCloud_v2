...
export default function ClientVendorDataGrid({
                                                 employerID,
                                                 privilege,
                                                 refreshRecord,
                                                 setRefreshRecord,
    ...
    async function ClientVendorListGet(uid) {
    ...
    const onSendReceiveClicked = (params) => {
        setOpenFileWindow(true);
    ...
    const columns = [
        {
            ...
            renderCell: (params) => {
                return (
                    <>
                        <IconButton
                            onClick={() => onSendReceiveClicked(params)}
                        >
                            {relationType === "Client" ? <CallReceivedIcon/> : <SendIcon/>}
                        </IconButton>
                        <IconButton
                            onClick={(event) => {
        ...
        {
            field: 'user',
            headerName: relationType === "Client" ? "Vendor User" : "Client User",
            width: columnWidth,
        },
        {
            field: 'business_name',
    ...
    useEffect(() => {
        const fetchData = async () => {
            await Promise.all([ClientVendorListGet(employerID)]);
        };
